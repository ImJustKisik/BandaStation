...
/datum/reagent/healium/on_mob_metabolize(mob/living/L)
	. = ..()
	if(L.IsSleeping())
		var/obj/item/organ/lungs/our_lungs = L.get_organ_slot(ORGAN_SLOT_LUNGS)
...
/datum/reagent/medicine/cryoxadone/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(M.bodytemperature < T0C)
		M.cause_pain(BODY_ZONES_ALL, -0.25 * power * REM * seconds_per_tick, 0.01)
...
/datum/reagent/medicine/painkiller/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	. = ..()
	if(current_cycle >= 5)
		SEND_SIGNAL(M, COMSIG_CARBON_MOOD_UPDATE, "numb", /datum/mood_event/narcotic_medium, name)
...
/datum/reagent/medicine/painkiller/paracetamol/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	M.adjust_bodytemperature(-12 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * seconds_per_tick, M.get_body_temp_normal())
	// Causes liver damage - higher dosages causes more liver damage.
	M.adjustOrganLoss(ORGAN_SLOT_LIVER, volume / 25 * REM * seconds_per_tick)
...
/datum/reagent/medicine/painkiller/paracetamol/overdose_process(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(M.disgust < 100 && SPT_PROB(100 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_disgust(5 * REM * seconds_per_tick)
...
/datum/reagent/medicine/painkiller/ibuprofen/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(M.disgust < DISGUST_LEVEL_VERYGROSS && SPT_PROB(50 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_disgust(max(current_cycle * 0.1, 5) * REM * seconds_per_tick)
	// ...and dizziness.
	if(current_cycle >= 25 && SPT_PROB(30 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_dizzy(1 * REM * seconds_per_tick)
...
/datum/reagent/medicine/painkiller/ibuprofen/overdose_process(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(85 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_dizzy(2 * REM * seconds_per_tick)
...
/datum/reagent/medicine/painkiller/aspririn_para_coffee/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	. = ..()

	// Heals all pain a bit if in low dosage. High metabolism, so it must make it count.
	if(volume <= 10)
		M.cause_pain(BODY_ZONES_ALL, -1 * REM * seconds_per_tick)
...
/datum/reagent/medicine/oxycodone/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	if(current_cycle >= 9)
		SEND_SIGNAL(M, COMSIG_CARBON_MOOD_UPDATE, "numb", /datum/mood_event/narcotic_heavy, name)
	...
	M.set_drugginess(10 * REM * seconds_per_tick)
	if(M.disgust < DISGUST_LEVEL_VERYGROSS && SPT_PROB(50 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_disgust(3 * REM * seconds_per_tick)
	if(SPT_PROB(33 * max(1 - creation_purity, 0.5), seconds_per_tick))
		M.adjust_dizzy(1 * REM * seconds_per_tick)
	. = ..()
	return TRUE

/datum/reagent/medicine/oxycodone/overdose_process(mob/living/carbon/M, seconds_per_tick, times_fired)
	. = ..()
	if(!ishuman(M))
		return

	var/mob/living/carbon/human/human_mob = M
	if(SPT_PROB(15 - (5 * normalise_creation_purity()), seconds_per_tick))
		...
		switch(picked_option)
			if(1)
				...
				human_mob.Paralyze(60 * REM * seconds_per_tick)
			if(2)
			...
			if(4)
				...
				human_mob.adjust_jitter(3 * REM * seconds_per_tick)
			if(5)
				to_chat(human_mob, span_danger("You feel the world spin."))
				human_mob.adjust_dizzy(3 * REM * seconds_per_tick)
			if(6)
...
/obj/machinery/vending/drugs
	premium = list(/obj/item/storage/pill_bottle/painkillers = 2)
